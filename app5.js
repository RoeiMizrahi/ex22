...
function isPrime(num) {
  if (num < 2) return false; // 0 ו-1 אינם מספרים ראשוניים
  for (let i = 2; i <= Math.sqrt(num); i++) if (num % i === 0) return false; // אם נמצא מחלק, המספר אינו ראשוני

  return true; // המספר ראשוני
}

// לולאה להדפסת כל המספרים הראשוניים מתחת ל-237
for (let i = 2; i < 237; i++) if (isPrime(i)) console.log(i); // הדפס את המספר אם הוא ראשוני
